...
let ticketPriceTotal = runners.reduce(function (total, element){
...
// Problem 1 === Companies who donate over $200 are going to be given a special mention in the newsletter. Create a new array with only the big spenders. (filter)

let highRollers =  runners.filter(function(element){
    return element.donation > 200;

})
console.log(highRollers)

// Problem 2 === Create a new array called mailing list based containing only the email addresses.

const mailingList = [];

runners.forEach(function(element){
    mailingList.push(element.email)
})
...
// Problem 3 === Remove all e-mail addresses to keep the privacy overlords happy. We can't store them in our database.

const privacyArr = runners.slice();

privacyArr.map(function(element){
